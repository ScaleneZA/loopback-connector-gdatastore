...
GDataStore.prototype.replaceById = function (model, id, data, options, callback, v) {
    debug('replaceById: %s %s %s %s', model, id, JSON.stringify(data), JSON.stringify(options));
    assert(id, 'Cannot update an entity without an existing id');
    this.createQuery(model, (query) => {
        query.update(id, data, (errors, result) => {
            this.callback(errors, result, callback);
...
GDataStore.prototype.save = function (model, data, options, callback) {
    debug('save: %s %s %s %s', model, JSON.stringify(data), JSON.stringify(options));

    const idName = this.idName(model);
    if (data[idName]) {
        this.updateAttributes(model, data[idName], data, options, callback);
    } else {
        this.create(model, data, options, callback);
    }
};

GDataStore.prototype.destroyAll = function (model, filter, options, callback) {
    debug('destroyAll: %s %s %s %s', model, JSON.stringify(filter), JSON.stringify(options));
    this.createQuery(model, (query) => {
        query.destroyAll((errors, result) => {
            this.callback(errors, result, callback);
        });
    });
};
